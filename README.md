...
- `includes/class-clc-xlsx-reader.php` — lector de `.xlsx` propio, sin dependencias.
- `includes/class-clc-importer.php` — importador de Excel: detecta filas de producto por contenido (no por posición fija de fila/columna), para tolerar encabezados corridos y notas al pie en el Excel del cliente.
...
## Instalar una versión nueva sin conflictos

WordPress no reemplaza automáticamente una carpeta de plugin por otra con nombre distinto (el zip trae el número
de versión en el nombre de la carpeta). Antes de subir un `.zip` nuevo, siempre hay que **desactivar y eliminar
la versión anterior** desde Plugins — si quedan dos carpetas activas a la vez, WordPress tira un error fatal
("Cannot redeclare...") porque ambas declaran las mismas funciones.
